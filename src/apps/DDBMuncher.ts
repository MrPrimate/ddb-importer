...
export default class DDBMuncher extends DDBAppV2 {
  ...
  _onRender(context, options) {
    ...
    this.element.querySelectorAll(".ddb-subclass-select").forEach((el) => {
      el.addEventListener("change", async (event) => {
        ...
        const classId = parseInt(target?.dataset.classId ?? "0");
        const selectedSubIds = DDBMuncher.getMultiSelectValues(event).map((id) => parseInt(id));
  ...
  static async parseFrames(this: DDBMuncher, _event, _target) {
    try {
      ...
      this.#notifyImportResult(`Finished importing ${result} frames!`,
        result === 0 ? DDBMuncher.#emptyImportMessage("frames") : undefined);
    } catch (error) {
      logger.error(error);
      logger.error(error.stack);
  ...
  async #parseOfficialClassesWithMule({ sourceIdArrays, baseOptions, classList, subclassSelections }: any = {}) {
  ...
  async _parseClassesWithMule() {
    ...
    try {
      ...
      // generate subclasses to parse (using the cached helper)
      for (const klass of classList) {
    ...
    if (!this.#muleHasDocuments) return DDBMuncher.#emptyImportMessage("classes");
  }
  ...
  async _parseWithMule(type) {
    ...
    if (!this.#muleHasDocuments) {
      return DDBMuncher.#emptyImportMessage(type === "species" ? "species" : `${type}s`);
    }
  }
