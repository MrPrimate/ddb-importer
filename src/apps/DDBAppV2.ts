...
const SETTING_RENDER_OPTIONS: TSettingRenderOptions = { ddbSettingRender: true };
...
export default abstract class DDBAppV2 extends HandlebarsApplicationMixin(ApplicationV2) {
  ...
  // pending setting writes, run one after another; a newer write with the same key supersedes
  // any queued write that has not run yet
   ...
   */
  async queueSettingUpdate(update: () => Promise<void>, { key = null, render = true }: {
    key?: string | null;
    render?: boolean;
  } = {}): Promise<void> {
    ...
    const scheduled = (async () => {
      try {
        while (this.rendered) {
          ...
          try {
            await this.render(SETTING_RENDER_OPTIONS);
            break;
          } catch (err) {
  ...
  /** @inheritDoc */
  async _preRender(context: DeepPartial<foundry.applications.api.Application.RenderContext>, options: DeepPartial<foundry.applications.api.Application.RenderOptions>) {
  ...
  /** @inheritDoc */
  _preSyncPartState(partId: string, newElement: HTMLElement, priorElement: HTMLElement, state: TDDBPartState) {
  ...
  /** @inheritDoc */
  _syncPartState(partId: string, newElement: HTMLElement, priorElement: HTMLElement, state: TDDBPartState) {
  ...
  notifierV2({ progress, section = "note", message = "", suppress = false, isError = false,
    ...
    if (clear && importProgressElement) importProgressElement.classList.add("munching-hidden");
