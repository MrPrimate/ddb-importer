...
interface ISourceBookBrowserContext extends Record<string, unknown> {
  searchTerm: string;
...
export default class DDBSourceBookBrowser extends DDBAppV2 {
  ...
  async _prepareContext(
    ...
    const context = await super._prepareContext({ ...options, noCacheLoad: true }) as unknown as ISourceBookBrowserContext;
