...
export default class DDBToolProficiencies {
  ...
  static getToolKey(tool: { baseTool?: string | null; toolKey?: string; name: string }): string {
    ...
    const toolKey = tool.toolKey ?? alias?.toolKey;
    if (toolKey && utils.getSetting<boolean>("add-ddb-tools")) return toolKey;
    const baseTool = tool.baseTool ?? alias?.baseTool ?? null;
    return baseTool ?? utils.idString(tool.name.toLowerCase());
  }
  ...
  static async #addIcons<T>(items: T[]): Promise<T[]> {
    try {
      return await Iconizer.updateIcons({
        documents: items as unknown as Record<string, unknown>[],
      }) as unknown as T[];
    } catch (error) {
  ...
  static async #getFallbackFolderId(compendium: CompendiumCollection.Any): Promise<string | null> {
    try {
      const folder = await CompendiumHelper.createFolder({
        ...
        entityType: utils.entityMap().get("tool") as "Item",
      });
  ...
  static async #createFallbackItems(
    ...
    try {
      ...
      const created = await Item.createDocuments(
        items as unknown as Parameters<typeof Item.createDocuments>[0],
  ...
  static async #repairBaseItems(
    ...
    try {
      if (wasLocked) compendium.configure({ locked: false });
      await Item.updateDocuments(updates as unknown as Parameters<typeof Item.updateDocuments>[0], { pack: compendium.collection });
