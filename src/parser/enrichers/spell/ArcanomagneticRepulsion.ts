 ...
 * metal, which no region filter can test: it slows enemies, and the behavior is there to switch
 * off for one that wears none. DDB records the area as a sphere, which would stay where it was
...
export default class ArcanomagneticRepulsion extends DDBEnricherData {
  ...
  override get activity(): IDDBActivityData {
    return castPlacer([DDBEnricherData.BehaviorHelper.difficultTerrain()], { target: emanation(RADIUS, "enemy") });
  }
