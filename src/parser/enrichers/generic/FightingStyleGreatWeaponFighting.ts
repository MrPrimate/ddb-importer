 ...
 *
 * dnd5e 6 rule changes can scope a rule to a two-handed melee weapon attack, but damage rolls only
 * collect `bonus` rules, so nothing native can change a die result. AC5e applies the die modifier
 * at roll time and reads the grip the attack was made with. Without AC5e, DDBItem bakes the same
 * modifier onto the weapon's damage parts instead, and stands down when AC5e is installed so the
 * two never stack.
 */
