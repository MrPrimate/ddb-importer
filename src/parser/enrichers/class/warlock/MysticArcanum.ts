...
export default class MysticArcanum extends DDBEnricherData {
  ...
  override get additionalAdvancements(): I5eAdvancement[] {
    ...
    const choices: Record<string, { count: number | null; replacement: boolean }> = {
  ...
  override async customFunction(_options: ICustomFunctionOptions) {
    ...
    // only the spells that resolved are done: one the compendium lacks is not munched yet, and
    // a later pass in the same session has to be able to try it again
    added.forEach((id) => MysticArcanum._listedSpellIds.add(id));
  }
