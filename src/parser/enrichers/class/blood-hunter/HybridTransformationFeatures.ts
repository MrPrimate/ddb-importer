...
/**
 * Order of the Lycan hybrid form. DDB ships no modifiers for this feature, and models the
 * transformation as a character builder choice, so everything is authored here as a self applying
 * enchantment: activating it grants the Predatory Strike activities and the Hybrid Form effect as
 * riders, activating it again removes them.
 *
 * Three enchantment profiles are level banded so Feral Might's damage bonus (+1/+2/+3) and the
 * Predatory Strike die (1d6, 1d8 from 11th) resolve without the player picking. Only one band is
 * ever valid, so dnd5e auto selects it rather than prompting.
 */
export default class HybridTransformationFeatures extends DDBEnricherData {
  ...
  _strikeActivities(denomination: number, ids: { action: string; bonus: string }): IDDBAdditionalActivity[] {
    const damageParts = [
      DDBEnricherData.basicDamagePart({
        customFormula: `1d${denomination} + ${HybridTransformationFeatures.DAMAGE_BONUS}`,
    ...
    const attack = {
      ...
      bonus: HybridTransformationFeatures.ATTACK_BONUS,
      type: {
        value: "melee",
  ...
  get additionalActivities(): IDDBAdditionalActivity[] {
    return [
      ...this._strikeActivities(6, HybridTransformationFeatures.STRIKE_IDS.small),
      ...this._strikeActivities(8, HybridTransformationFeatures.STRIKE_IDS.large),
      {
        init: {
          name: "Bloodlust",
        ...
        overrides: {
          id: HybridTransformationFeatures.BLOODLUST_ID,
          targetType: "self",
          data: {
            range: { units: "self" },
            duration: { units: "inst" },
    ...
    return {
      ...
      data: {
        flags: {
          ddbimporter: {
            effectIdLevel: level,
            activityRiders: [strikeIds.action, strikeIds.bonus, HybridTransformationFeatures.BLOODLUST_ID],
  ...
  get effects(): IDDBEffectHint[] {
    const bands = [
      { damageBonus: 1, effectId: "ddbLycanForm0001", strikeIds: HybridTransformationFeatures.STRIKE_IDS.small, level: { min: null, max: 10 } },
      { damageBonus: 2, effectId: "ddbLycanForm0002", strikeIds: HybridTransformationFeatures.STRIKE_IDS.large, level: { min: 11, max: 17 } },
      { damageBonus: 3, effectId: "ddbLycanForm0003", strikeIds: HybridTransformationFeatures.STRIKE_IDS.large, level: { min: 18, max: null } },
    ];
