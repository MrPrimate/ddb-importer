...
export default class _CrimsonRite extends _BloodHunter {
   ...
   * legacy `[[formula, type]]` pair or a whole DamageData object.
   *
   * A reroll modifier has to sit on a literal die term, and a bare `@scale` reference has none,
   * so the rerolled die is spelled out from the scale's `faces` in a custom formula.
   *
  ...
  riteDamagePartValue(riteName: string | null, damageType: string): string {
    if (riteName === "Rite of the Flame" && this.hasFiendRiteFocus) {
      return JSON.stringify({
        custom: { enabled: true, formula: `1d(${this.riteDie.replace(/\.die$/, ".faces")})r<=2` },
        types: [damageType],
      });
    }
    return damageType
