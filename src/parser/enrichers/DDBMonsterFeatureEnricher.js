...
import FlyingSnakeBite from "./monster/FlyingSnakeBite.js";

export default class DDDMonsterFeatureEnricher extends DDBBaseEnricher {

  _loadEnricherData(name) {
    const stub = this.ENRICHERS?.[this.monsterName]?.[name];
    if (stub) {
      const EnricherClass = DDBBaseEnricher._loadDataStub(stub);
