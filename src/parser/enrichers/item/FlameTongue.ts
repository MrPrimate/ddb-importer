    ...
    return [
      itemActivity(FlameTongue.ACTIVITY_NAME, DDBEnricherData.ACTIVITY_TYPES.ENCHANT, {
        ...
        data: {
          ...
          // an applied effect with no expiry inherits the activity's duration, and the flames are open-ended
          duration: {
            value: "",
            units: "spec",
  ...
  override get effects(): IDDBEffectHint[] {
    return [
      ...
      },
      {
        ...
        tokenChanges: [
          DDBEnricherData.ChangeHelper.tokenChange("token.light.bright", "upgrade", 40, 20),
          DDBEnricherData.ChangeHelper.tokenChange("token.light.dim", "upgrade", 80, 20),
          DDBEnricherData.ChangeHelper.tokenChange("token.light.color", "override", "#f8c377", 20),
          DDBEnricherData.ChangeHelper.tokenChange("token.light.alpha", "override", "0.4", 20),
          DDBEnricherData.ChangeHelper.tokenChange("token.light.animation.type", "override", "torch", 20),
          DDBEnricherData.ChangeHelper.tokenChange("token.light.animation.speed", "override", "2", 20),
          DDBEnricherData.ChangeHelper.tokenChange("token.light.animation.intensity", "override", "2", 20),
        ],
      },
    ];
  }
