 ...
 * stock SRD actor. It is placed from the item instead: an emanation is attached to whichever token
 * the user clicks, so the rider places it on the goat, and it fires a save kept on the item.
 */
  ...
  get auraActivities(): IDDBAdditionalActivity[] {
    ...
    return [
      regionPlacer(`${aura.name}: Aura`, {
        ...
        behaviors: [
          DDBEnricherData.BehaviorHelper.activity({ events: aura.events, activityName: saveName, excludeSelf: true }),
        ],
      }),
      regionTrigger(saveName, {
        affects: aura.affects,
