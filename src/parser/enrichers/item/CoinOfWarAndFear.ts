 ...
 * emanation for 1 minute; its Wisdom save is rolled by hand against an enemy that ends its turn
 * inside.
 */
export default class CoinOfWarAndFear extends DDBEnricherData {
  ...
  override get additionalActivities(): IDDBAdditionalActivity[] {
    return [
      ...
      {
        init: { name: "Boon of Beleth", type: DDBEnricherData.ACTIVITY_TYPES.UTILITY },
        build: {
          generateSave: false,
          ...
          activationOverride: {
            type: "special",
            value: null,
            condition: "The coin shows Beleth (odd)",
          },
          targetOverride: {
            override: true,
            affects: { type: "enemy" },
      ...
      {
        init: { name: "Boon of Beleth Save", type: DDBEnricherData.ACTIVITY_TYPES.SAVE },
        build: {
          generateSave: true,
          generateDamage: false,
          generateActivation: true,
          ...
          generateTarget: true,
          generateRange: true,
          saveOverride: { ability: ["wis"], dc: { calculation: "", formula: "14" } },
          activationOverride: {
            type: "special",
            value: null,
      ...
      {
        init: { name: "Boon of Beleth: Psychic Damage", type: DDBEnricherData.ACTIVITY_TYPES.DAMAGE },
        build: {
          generateSave: false,
          generateDamage: true,
          generateActivation: true,
  ...
  override get effects(): IDDBEffectHint[] {
    return [
      ...
      },
      {
        ...
        options: {
          transfer: false,
          // applied as the target's own turn ends, so one counted round would stop short of its next turn
          expiry: "targetEnd",
          durationRounds: 2,
          durationSeconds: 12,
