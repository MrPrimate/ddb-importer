...
export function describeMissingItemSpells(missing: I5eSpellItem[]): string {
  ...
  const lines = missing.map((spell) => {
    const book = DDBSources.getDocumentSourceBookName(spell as unknown as TAll5eDocuments);
    books.add(book);
...
function rawEntriesFor(ddb: IDDBData, missing: I5eSpellItem[]): IDDBSpellEntry[] {
  ...
  for (const entry of [...(ddb.character.spells.item ?? []), ...(ddb.unequippedItemSpells ?? [])]) {
...
async function munchMissingItemSpells(ddb: IDDBData, missing: I5eSpellItem[], {
...
}: { notifier?: TItemsNotifier | null; generateSummons?: boolean | null } = {}): Promise<void> {
  ...
  const handler = new DDBItemImporter<I5eSpellItem>("spells", documents, {
