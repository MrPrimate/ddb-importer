...
export function getLairActions(monster, DDB_CONFIG) {
  ...
  const dom = JSDOM.fragment(monster.lairDescription);
  ...
  let defaultAction = JSON.parse(JSON.stringify(require("./templates/feat.json")));
  defaultAction.name = "Lair Actions";
  ...
  dom.childNodes.forEach((node) => {
    ...
    if (initiativeMatch) {
      resource.initiative = parseInt(initiativeMatch[1]);
    }
  });
