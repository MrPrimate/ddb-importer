...
const BANNED_PATTERNS: { name: string; pattern: RegExp; reason: string }[] = [
  {
    name: "numeric change mode",
    pattern: /\bmode:\s*(\d|CONST\.ACTIVE_EFFECT_MODES)/,
    reason: "v14 effect changes use string `type` (\"add\", \"override\", ...), not numeric `mode`",
  },
  {
    name: "top-level changes in update payload",
    pattern: /\.update\(\s*\{\s*(["']?)changes\1\s*:/,
    reason: "Document#update diffs skip migrateData - top-level `changes` is silently dropped; write `system.changes`",
  },
  {
    name: "movement.all",
    pattern: /movement\.all\b/,
    reason: "dnd5e 6.0 uses movement.bonus (add) / movement.multiplier (multiply); reads use movement.speeds.*",
  },
  {
    name: "removed roll API",
    pattern: /rollAbilitySave|rollAbilityTest/,
    reason: "removed in dnd5e 6.0 - use rollSavingThrow({ability}) / rollAbilityCheck({ability})",
  },
  {
    name: "logger global",
    pattern: /\blogger\./,
  ...
  {
    name: "legacy deletion key",
    pattern: /["']-=/,
    reason: "v14 deprecated {\"-=key\": null} - use the ForcedDeletion `_del` operator as the value",
  },
  {
