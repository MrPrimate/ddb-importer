...
describe("muncher empty import notices", () => {
  beforeEach(() => {
    ...
    vi.spyOn(DDBSources, "getChosenSourceIdSet").mockReturnValue(new Set([2]));
  ...
  it.each(actions)("reports an empty %s import without a success message", async (type, action) => {
    ...
    if (type === "frames") {
      expect(app.notifierV2).toHaveBeenCalledWith(expect.objectContaining({ message: notice }));
    } else {
  ...
  it.each(actions)("preserves successful %s completion", async (type, action) => {
    ...
    expect(ui.notifications.error).not.toHaveBeenCalled();
    if (type === "frames") {
      expect(app.notifierV2).toHaveBeenCalledWith(expect.objectContaining({ message: "Finished importing 1 frames!" }));
    } else {
      const counted = type === "monsters" || type === "vehicles" ? "1 " : "";
  ...
  it.each(["parseFeats", "parseBackgrounds"] as const)("reports when %s excludes every existing entry", async (action) => {
    mockResults(1);
    setMockSettings({ "munching-policy-character-dont-grab-existing": true });
    vi.spyOn(DDBMuleHandler, "getExistingFeatIds").mockResolvedValue(new Set([10]));
    vi.spyOn(DDBMuleHandler, "getExistingBackgroundIds").mockResolvedValue(new Set([10]));
    await DDBMuncher[action].call(makeMuncher(), null, null);
    expect(DDBMuleHandler.prototype.process).not.toHaveBeenCalled();
    expect(ui.notifications.info).toHaveBeenCalledWith(expect.stringContaining("were available to import"));
    expect(ui.notifications.info).toHaveBeenCalledWith(expect.stringContaining("skips entries already imported"));
  });

  it.each([false, true])("reports when all selected subclasses already exist (homebrew=%s)", async (homebrew) => {
    mockResults(1);
    setMockSettings({
      "munching-policy-character-dont-grab-existing": true,
    ...
    vi.spyOn(DDBMuleHandler, "getExistingSubclassIds").mockResolvedValue(new Set([10]));
    await DDBMuncher.parseClasses.call(makeMuncher(), null, null);
    expect(DDBMuleHandler.prototype.process).not.toHaveBeenCalled();
    expect(ui.notifications.info).toHaveBeenCalledWith(expect.stringContaining("No new classes"));
    expect(ui.notifications.info).toHaveBeenCalledWith(expect.stringContaining("skips entries already imported"));
  });

  it.each([false, true])("reports when all selected species already exist (homebrew=%s)", async (homebrew) => {
    mockResults(1);
    setMockSettings({
      "munching-policy-character-dont-grab-existing": true,
      "munching-policy-character-fetch-homebrew": homebrew, "munching-policy-character-only-homebrew": homebrew,
    });
    vi.mocked(DDBMuleHandler.getList).mockResolvedValue([{ ...species, isHomebrew: homebrew }]);
    vi.spyOn(DDBMuleHandler, "getExistingSpeciesKeys").mockResolvedValue(new Set(["1743923279:4"]));
    await DDBMuncher.parseSpecies.call(makeMuncher(), null, null);
    expect(DDBMuleHandler.prototype.process).not.toHaveBeenCalled();
    expect(ui.notifications.info).toHaveBeenCalledWith(expect.stringContaining("No new species"));
    expect(ui.notifications.info).toHaveBeenCalledWith(expect.stringContaining("skips entries already imported"));
  });
