...
describe("DDBAppV2 progress bars", () => {
  ...
  it("clears a bar without needing another progress count", () => {
    const app = buildApp();
    app.notifierV2({ progress: { current: 1, total: 2 }, message: "working", suppress: true });
    app.notifierV2({ message: "", clear: true, suppress: true });
    expect(barHidden(app, "primary")).toBe(true);
  });

  it("handles empty totals and clamps out-of-range progress", () => {
    const app = buildApp();
    app.notifierV2({ progress: { current: 0, total: 0 }, message: "empty", suppress: true });
    expect(barWidth(app, "primary")).toBe("0%");
    app.notifierV2({ progress: { current: 3, total: 2 }, message: "done", suppress: true });
    expect(barWidth(app, "primary")).toBe("100%");
  });
