...
function makeFakeAdvancement(type: string) {
  return class FakeAdvancement {
    ...
    // dnd5e's BaseAdvancement exposes its fields directly; the link builders read the id before toObject()
    get _id() {
...
// this branch ships the fallback config as TypeScript modules rather than JSON assets
import { fallbackDDBConfig } from "../../src/hooks/ready/fallbackConfig";
import { RULE_DATA } from "../../src/hooks/ready/fallbackRules";

const fallbackRuleData = RULE_DATA as unknown as IDDBRuleData;
...
(globalThis as any).CONST = {
  DEFAULT_TOKEN: "icons/svg/mystery-man.svg",
  // Foundry 13 active effect change modes; enrichers on this branch emit numeric modes
