...
describe("EffectGenerator speed bonuses", () => {
  ...
  it("skips a null speed bonus with no restriction instead of emitting NaN", () => {
    ...
    expect(generator.effect.changes).toHaveLength(0);
  });
  ...
  it("sets a speed from a distance in the restriction", () => {
    ...
    expect(generator.effect.changes).toEqual([
      expect.objectContaining({ key: "system.attributes.movement.fly", value: 20, mode: CONST.ACTIVE_EFFECT_MODES.UPGRADE }),
    ]);
  });
