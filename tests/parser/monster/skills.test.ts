...
import { logger, utils } from "../../../src/lib/_module";
...
describe.each(["_generateSkills", "_generateSkillsHTML"] as const)("DDBMonster.%s additional bonuses", (method) => {
  ...
  it.each(["", "5"])("applies the roll bonus without overwriting passive-only bonus %j", (passiveBonus) => {
    ...
    expect(skill.roll.bonus).toBe("3");
    // The system includes the roll bonus in passive scores automatically.
...
describe("DDBMonster._generateSkills", () => {
  ...
  it("skill with additionalBonus", () => {
    ...
    expect(mock.npc.system.skills.prc.bonuses.passive).toBe("");
  });
