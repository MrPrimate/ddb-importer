...
describe("class-feature damage scales", () => {
  ...
  it.each([false, true])("uses the owning class levels for the emitted advancement (subclass: %s)", (subclass) => {
    ...
    expect(advancement).toMatchObject({ configuration: {
      ...
      type: "dice",
      scale: {
        3: { number: 1, faces: 6 }, 5: { number: 2, faces: 6 },
        11: { number: 3, faces: 6 }, 17: { number: 4, faces: 6 },
      },
    } });
  ...
  it("writes the link into the generated damage activity", () => {
    ...
    expect(activity.buildData.damage?.parts?.[0].custom).toEqual({ enabled: true, formula: "@scale.fighter.scaling-strike" });
  });
  ...
  it.each([
    ...
    await parser.enricher._applyActivityDataOverride(activity.buildData, hint);
    expect(activity.buildData.damage?.parts).toHaveLength(1);
    expect(activity.buildData.damage?.parts?.[0].types).toEqual([type]);
    if (formula) {
      expect(activity.buildData.damage?.parts?.[0].custom?.formula).toBe(formula);
    } else {
      expect(activity.buildData.damage?.parts?.[0].bonus).toBe("@scale.the-predator.thrill-of-the-hunt");
    }
  });
  ...
  it.each([
    ...
    const activity = new DDBFeatureActivity({ type: "damage", ddbParent: parser });
    activity._generateDamage();
    await parser.enricher._applyActivityDataOverride(activity.buildData, makeEnricherData(Enricher).activity);
    expect(activity.buildData.damage?.parts).toHaveLength(1);
    expect(activity.buildData.damage?.parts?.[0]).toMatchObject({
