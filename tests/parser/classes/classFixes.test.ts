...
function scales(stub: any): Record<string, any> {
  return Object.fromEntries(
    (Object.values(stub.data.system.advancement) as any[])
      ...
      .map((a) => [a.configuration.identifier, { name: a.title, type: a.configuration.type, scale: a.configuration.scale }]),
  );
}
...
describe("DDBClass scale fixes", () => {
  ...
  it("adds Rage Damage only when the class did not already generate it", () => {
    ...
    already._addAdvancement({ _id: "existingRage0000", type: "ScaleValue", title: "Rage Damage", configuration: { identifier: "rage-damage", type: "number", scale: {} } });
